...
public class VideoActivity extends PlaybackActivity implements CustomKeyDownVod.Listener, TrackDialog.Listener, SkipDialog.Listener, ArrayAdapter.OnClickListener, FlagAdapter.OnClickListener, EpisodeAdapter.OnClickListener, QualityAdapter.OnClickListener, QuickAdapter.OnClickListener, ParseAdapter.OnClickListener, Clock.Callback {
    ...
    private Site getSite() {
        ...
        if (!site.isEmpty() || !getKey().startsWith("local_media_")) return site;
        Site local = Site.find(getKey());
    ...
    private void setVideoView() {
        mBinding.control.action.danmakuShow.setVisibility(DanmakuSetting.isLoad() ? View.VISIBLE : View.GONE);
    ...
    private void setPlayer(Result result) {
        ...
        if (result.hasDesc()) {
            ...
            mBinding.content.setText(result.getDesc());
        }
    ...
    public void onItemClick(Result result) {
        updateHistoryProgress(System.currentTimeMillis(), false);
        startPlayer(getHistoryKey(), result, isUseParse(), getSite().getTimeout(), buildMetadata(), getStartPosition());
    }
    ...
    private void checkDanmakuStatus() {
        String status = ResUtil.getString(DanmakuSetting.isShow() ? R.string.setting_on : R.string.setting_off);
        mBinding.control.action.danmakuShow.setText(ResUtil.getString(R.string.danmaku_status, status));
    }
    ...
    private void updateAlwaysTime() {
        if (mBinding.alwaysTimeText == null) return;
        boolean show = com.fongmi.android.tv.setting.Setting.isAlwaysTime()
                && !isVisible(mBinding.control.getRoot())
                && !isVisible(mBinding.widget.top)
                && !isVisible(mBinding.widget.center);
        if (show) {
            mBinding.alwaysTimeText.setVisibility(View.VISIBLE);
            mBinding.alwaysTimeText.setText(java.time.LocalTime.now().format(com.fongmi.android.tv.utils.Formatters.TIME));
            App.removeCallbacks(mClockRunnable);
            App.post(mClockRunnable, 1000);
        } else {
            mBinding.alwaysTimeText.setVisibility(View.GONE);
            App.removeCallbacks(mClockRunnable);
        }
    }
    ...
    public void onTimeChanged(long time) {
        ...
        if (!updateHistoryProgress(time, true)) return;
    ...
    private boolean updateHistoryProgress(long time, boolean requirePlaying) {
        if (mHistory == null || !isOwner()) return false;
        if (requirePlaying && !player().isPlaying()) return false;
        long position = player().getPosition();
        long duration = player().getDuration();
    ...
    private long getStartPosition() {
        ...
        long position = Math.max(mHistory.getOpening(), Math.max(0, mHistory.getPosition()));
