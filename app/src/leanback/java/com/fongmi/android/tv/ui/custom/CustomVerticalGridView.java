...
public class CustomVerticalGridView extends VerticalGridView {
    ...
    public boolean isHeaderVisible() {
        if (views != null) {
            for (View view : views) {
                if ((view.getId() == R.id.recycler || view.getId() == R.id.recyclerV) && view.getVisibility() == View.VISIBLE) return true;
            }
        }
        return false;
    }
    ...
    public boolean moveToTop() {
        if (getSelectedPosition() == 0 || getAdapter() == null || getAdapter().getItemCount() == 0) return false;
        int targetId = Setting.getSearchMode() == 0 ? R.id.recycler : R.id.recyclerV;
        Activity activity = com.fongmi.android.tv.App.activity();
        if (activity != null) {
