...
public class LiveActivity extends PlaybackActivity implements GroupAdapter.OnClickListener, ChannelAdapter.OnClickListener, EpgDataAdapter.OnClickListener, CustomKeyDownLive.Listener, CustomLiveListView.Callback, TrackDialog.Listener, PassListener, ConfigListener, LiveListener {
    ...
    protected void onServiceConnected() {
        ...
        if (player() != null && player().getPlayer() != null) {
            player().getPlayer().setVolume(1.0f);
        }
        checkLive();
    }
    ...
    protected void onSizeChanged(VideoSize size) {
        mBinding.widget.videoSize.setText(player().getSizeText());
    }
    ...
    private void updateAlwaysTime() {
        ...
        boolean show = com.fongmi.android.tv.setting.Setting.isAlwaysTime()
                ...
                && !isVisible(mBinding.widget.bottom)
    ...
    private void setInfo() {
        ...
        mBinding.widget.timeRange.setText("");
        mBinding.widget.timeRange.setVisibility(View.GONE);
        mBinding.widget.videoSize.setText(player().getSizeText());
    ...
    private void setEpg(Epg epg) {
        ...
        mBinding.widget.play.setText(data.getTitle());
        String time = data.getTime();
        mBinding.widget.timeRange.setText(time);
        mBinding.widget.timeRange.setVisibility(time.isEmpty() ? View.GONE : View.VISIBLE);
        setWidth(epg);
        setMetadata();
        notifyItemChanged(mBinding.channel, mChannelAdapter);
    }
    ...
    private void fetch() {
        if (mChannel == null) return;
        android.util.Log.d("AntigravityLive", "fetch called for channel: " + mChannel.getName());
        LiveConfig.get().setKeep(mChannel);
        if (player().isPlaying() && !player().isEmpty() && mChannel.getName().equals(mPlayingName)) {
            android.util.Log.d("AntigravityLive", "fetch: channel already playing, return early");
            player().getPlayer().setVolume(1.0f);
            setArtwork();
            showInfo();
            hideProgress();
    ...
    private void start(Result result) {
        android.util.Log.d("AntigravityLive", "start called with result url: " + result.getRealUrl() + ", error/msg: " + result.getMsg());
        mPlaybackKey = result.getRealUrl();
        mPlayingName = mChannel != null ? mChannel.getName() : null;
    ...
    protected void onStop() {
        ...
        mClockHhmm.stop();
        mClockSs.stop();
        if (isOwner() && controller() != null) controller().pause();
    }
    ...
    protected void onDestroy() {
        ...
        mClockSs.release();
        Source.get().exit();
        App.removeCallbacks(mR0, mR1, mR2, mR3, mR4, mClockRunnable);
