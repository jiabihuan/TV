...
public class Site implements Parcelable {
    ...
    public Style getStyle() {
        if (style == null && !TextUtils.isEmpty(getExt())) {
            try {
                android.net.Uri uri = android.net.Uri.parse(getExt());
                String styleVal = uri.getQueryParameter("style");
                if (!TextUtils.isEmpty(styleVal)) {
                    style = new Style(styleVal);
                }
    ...
    public void delete() {
        AppDatabase.get().getSiteDao().delete(getKey());
    }
